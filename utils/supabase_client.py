...
class SupabaseDB:
    """Client Supabase REST léger, compatible avec les clés sb_* et eyJ*."""
...
class TableQuery:
    ...
    def select(self, columns: str = "*", *, count: str = None, head: bool = False):
...
def get_supabase() -> SupabaseDB:
    """Client avec service_role key — uniquement pour les opérations admin."""
...
def get_supabase_for_user(user_token: str) -> SupabaseDB:
    """Client avec clé anon + JWT utilisateur — RLS actif, sécurisé."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise RuntimeError("Supabase non configuré. Remplis SUPABASE_URL et SUPABASE_KEY dans .env")
    db = SupabaseDB(SUPABASE_URL, SUPABASE_KEY)
    # Remplace l'Authorization par le JWT de l'utilisateur pour activer le RLS

...
def _generer_numero(user_id: str, token: str) -> str:
    ...
    prefix = f"DEV-{year}-"
    result = get_supabase_for_user(token).table("devis").select("numero").eq("user_id", user_id).like("numero", f"{prefix}%").order("numero", desc=True).limit(1).execute()
    if result.data:
...
async def list_devis(current_user: dict = Depends(get_current_user)):
    result = get_supabase_for_user(current_user["token"]).table("devis").select("*, clients(nom, prenom, email)").eq("user_id", current_user["id"]).order("date_creation", desc=True).execute()
    return result.data
...
async def get_devis(devis_id: str, current_user: dict = Depends(get_current_user)):
    result = get_supabase_for_user(current_user["token"]).table("devis").select("*, clients(nom, prenom, email, telephone, adresse)").eq("id", devis_id).eq("user_id", current_user["id"]).single().execute()
    if not result.data:
...
async def update_devis(devis_id: str, data: DevisUpdate, current_user: dict = Depends(get_current_user)):
    ...
    result = db.table("devis").update(update_data).eq("id", devis_id).eq("user_id", current_user["id"]).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Devis non trouvé")
    return result.data[0]
...
async def delete_devis(devis_id: str, current_user: dict = Depends(get_current_user)):
    db = get_supabase_for_user(current_user["token"])
    check = db.table("devis").select("id").eq("id", devis_id).eq("user_id", current_user["id"]).execute()
    if not check.data:
        raise HTTPException(status_code=404, detail="Devis non trouvé")
...
async def envoyer_devis(devis_id: str, current_user: dict = Depends(get_current_user)):
    db = get_supabase_for_user(current_user["token"])
    devis = db.table("devis").select("*, clients(email, nom, prenom)").eq("id", devis_id).eq("user_id", current_user["id"]).single().execute().data
    if not devis:
        raise HTTPException(status_code=404, detail="Devis non trouvé")
    ...
    db.table("devis").update({"statut": "envoyé", "date_envoi": datetime.now(timezone.utc).isoformat(), "acceptance_token": acceptance_token}).eq("id", devis_id).execute()
...
async def get_devis_public(acceptance_token: str):
    db = get_supabase()
    result = db.table("devis").select("id, numero, titre, prestations, montant_ht, tva, montant_ttc, statut, date_creation, date_validite, notes, user_id, clients(nom, prenom, adresse)").eq("acceptance_token", acceptance_token).execute()
    if not result.data:
    ...
    profile = db.table("profiles").select("stripe_enabled, moyens_paiement, instructions_paiement, entreprise, prenom, nom").eq("id", devis["user_id"]).single().execute()
...
async def accepter_devis_public(acceptance_token: str):
    ...
    db.table("devis").update({"statut": "accepté", "date_acceptation": datetime.now(timezone.utc).isoformat()}).eq("acceptance_token", acceptance_token).execute()
    if devis.get("client_id"):

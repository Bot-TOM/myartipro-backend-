...
async def list_devis(current_user: dict = Depends(get_current_user)):
    result = await (
        get_supabase_for_user(current_user["token"]).table("devis")
...
async def get_devis_public(acceptance_token: str):
    ...
    profile = await (
        db.table("profiles")
        .select("stripe_enabled, moyens_paiement, instructions_paiement, entreprise, prenom, nom")
        .eq("id", devis["user_id"])
        .single()
        .execute()
    )
